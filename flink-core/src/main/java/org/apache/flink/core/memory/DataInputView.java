 ...
 * org.apache.flink.core.memory.MemorySegment}. 内存页是MemorySegment之上的数据访问视图，数据读取抽象为
 * DataInputView，数据写入抽象为DataOutputView。有了这一层，上层使用者无须关心MemorySegment的细节，该层会自动处理
 * 跨MemorySegment的读取和写入。
 *
 * <p>InputView 中 持 有 多 个 MemorySegment 的 引 用
 * （ MemorySegment ［ ］ ） ， 这 一 组 MemorySegment 被 视 为 一 个 内 存 页
 * （Page），可以顺序读取MemorySegment中的数据。</p>
 */
@Public
