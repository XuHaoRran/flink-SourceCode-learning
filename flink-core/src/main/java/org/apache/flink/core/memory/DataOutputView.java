 ...
 * DataOutputView是数据写入MemorySegment的抽象视图，继承自java.io.DataOutput，提供了将不同类型的数据写入二进制流的一系
 * 列方法。同样，DataOutputView中持有一个或者多个MemorySegment的引用（MemorySegment［］），这一组MemorySegment被视为一个内存
