 ...
 *
 * <ul>
 *   <li>It offers additional binary compare, swap, and copy methods.它提供了额外的二进制比较、交换和复制方法。
 *   <li>It uses collapsed checks for range check and memory segment disposal.它使用折叠检查进行范围检查和内存段处理。
 *   <li>It offers absolute positioning methods for bulk put/get methods, to guarantee thread safe
 *       use.它为批量put/get方法提供了绝对定位方法，以确保线程安全使用。
 *   <li>It offers explicit big-endian / little-endian access methods, rather than tracking
 *       internally a byte order.它提供了显式的大端/小端访问方法，而不是在内部跟踪字节顺序。
 ...
 * ...) are read and written with "unsafe" native commands.对实现的评论：我们大量使用本机指令支持的操作，
 * 以实现高效率。多字节类型（int、long、float、double…）使用“不安全”本地命令进行读写。
 *
 ...
 * implementations on invocations of abstract methods. 关于效率的注意事项：为了获得最佳效率，
 * 我们不将不同内存类型的实现与继承分开，以避免在抽象方法调用时查找具体实现的开销。
 *
 * <p>在Flink中，Java对象的有效信息被序列化为二进制数据流，在内
 * 存 中 连 续 存 储 ， 保 存 在 预 分 配 的 内 存 块 上 ， 内 存 块 叫 作
 * MemorySegment。MemorySegment是内存分配的最小单元，是一段固定
 * 长度的内存（默认大小为32KB），并且提供了非常高效的读写方法，
 * 很多运算可以直接操作二进制数据，不需要反序列化即可执行。</p>
 *
 * <p>MemorySegment可以保存在堆上，其内部存储为一个Java byte数
 * 组，也可以保存在堆外的ByteBuffer中。每条记录都会以序列化的形
 * 式存储在一个或多个MemorySegment中。</p>
 */
@Internal
public final class MemorySegment {
     ...
     * use this well to aggressively eliminate the non-applicable code paths. 判断是否为Little Endian
     * 模式的字节存储顺序，若不是，就是Big Endian模式
     */
     ...
     * 如果MemeorySegment使用堆上内存，则表示一个堆上的字节数组（byte［］），
     * 如果MemorySegment使用堆外内存，则为null。
     ...
     * 字节数组对应的相对地址（若HeapMemory为null，即可能为堆外内存的绝对地址）。
     */
    private long address;

    /**
     * The address one byte after the last addressable byte, i.e. <tt>address + size</tt> while the
     * segment is not disposed. 标识地址结束位置（address+size）
     */
    private final long addressLimit;

    /** The size in bytes of the memory segment. 内存段的字节数 */
