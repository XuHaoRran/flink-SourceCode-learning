 ...
 *
 *
 * <p>Buffer的底层是MemorySegment，Buffer申请和释放由Flink自行
 ...
 *
 * <p>
 *     Task算子处理数据完毕，将结果交给下游的时候，使用的抽象或者说内存对象是Buffer。Buffer接口是网络层面上传输数据和事件的
 * 统一抽象，其实现类是NetworkBuffer。Flink在各个TaskManager之间传递数据时，使用的是这一层的抽象。
 * 1个NetworkBuffer中包装了1个MemorySegment。
 * </p>
 */
