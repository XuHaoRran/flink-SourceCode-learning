...
final class BoundedBlockingSubpartition extends ResultSubpartition {
    ...
    /** The current buffer, may be filled further over time. 当前正在填充的BufferConsumer对象 */
    @Nullable private BufferConsumer currentBuffer;

    /** The bounded data store that we store the data in. 真正写入数据的入口，其多个实现类代表多种不同的数据
     * 存储方式 */
    private final BoundedData data;

    /** All created and not yet released readers. 表示所有创建的并且尚未被释放的子分区数据读取器 */
    @GuardedBy("lock")
    ...
    /** Counter for the number of data buffers (not events!) written. 表示写入的non-event buffer的个数 */
    ...
    public ResultSubpartitionView createReadView(BufferAvailabilityListener availability)
        ...
        synchronized (lock) {
            ...
            if (useDirectFileTransfer) {
                ...
                /** 构造BoundedBlockingSubpartitionReader的对象 **/
                reader =
