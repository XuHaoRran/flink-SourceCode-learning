...
public class PipelinedSubpartition extends ResultSubpartition
     ...
     * 表示该子分区的所有BufferConsumer队列 */
    final PrioritizedDeque<BufferConsumerWithPartialRecordLength> buffers =
            new PrioritizedDeque<>();

    /** The number of non-event buffers currently in this subpartition. 表示目前该子分区中non-event buffer的个数 */
    @GuardedBy("buffers")
    private int buffersInBacklog;

    /** The read view to consume this subpartition. 表示该子分区的试图，用于读取数据 */
