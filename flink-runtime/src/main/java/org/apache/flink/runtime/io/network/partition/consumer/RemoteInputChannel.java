...
public class RemoteInputChannel extends InputChannel {
    ...
    /** The connection to use to request the remote partition. 网络链接的唯一标识 */
    private final ConnectionID connectionId;

    /** The connection manager to use connect to the remote partition provider. 用于创建网络链接的客户端 */
    private final ConnectionManager connectionManager;

    /**
     * The received buffers. Received buffers are enqueued by the network I/O thread and the queue
     * is consumed by the receiving task thread. 将远程获取的Buffer放入到该队列，并被消费者消费
     */
    ...
    /** Client to establish a (possibly shared) TCP connection and request the partition. 网络链接的客户端 */
    private volatile PartitionRequestClient partitionRequestClient;

    /** The next expected sequence number for the next buffer. */
    private int expectedSequenceNumber = 0;

    /** The initial number of exclusive buffers assigned to this channel. 表示初始情况下下游可以接受的Buffer个数
     * 这个值会被封装到PartitionRequest中一起发送到上游，赋值给CreditBasedSequenceNUmberingViewReader对象
     * 的numCreditsAvaialable字段*/
