...
public class BatchTask<S extends Function, OT> extends AbstractInvokable
    ...
    /**
     * The driver that invokes the user code (the stub implement
     * ation). The central driver in this
     * task (further drivers may be chained behind this driver).
     * 不同类型的任务会又不同的Driver实现类来执行
     */
    ...
    /** The serializers for the input data type. 表示输入数据的序列化器的工厂类 */
    ...
    /** The comparators for the central driver. 表示输入数据的比较器 */
                    ...
                    Sorter<?> cSorter =
                            ExternalSorter.newBuilder(
                                    ...
                                    .withCombiner(
                                            ...
                                            this.config.getStubParameters()) // 初始化函数
                                    .enableSpilling(
                                            getIOManager(),
                                            this.config.getSpillingThresholdInput(inputNum))
                                    .memoryFraction(this.config.getRelativeMemoryInput(inputNum))
     ...
     * 核心逻辑可以概括为实例化算子驱动、初始化算子驱动，这时一个循环的过程，每次初始化一个算子驱动时都会将上一个算子
     * 驱动作为参数传入，一次维护算子间输入输出的关系。在构造连接中最后一个算子驱动时，调用getOutputCollector方法构造
     * 了它的输出，该方法的返回值的类型为OutputCollector，该类不是算子驱动，仅表示数据的输出
     */
    @SuppressWarnings("unchecked")
