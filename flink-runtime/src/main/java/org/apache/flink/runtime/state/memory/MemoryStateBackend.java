 ...
 * 运行时所需要的State数据保存在TaskManager JVM堆上内存中，KV类型的State、窗口算子的State
 * 使用HashTable来保存数据、触发器等。执行检查点的时候，会把State的快照数据保存到JobManager进程的内存
 * 中。MemoryStateBackend可以使用异步的方式进行快照，（也可以使用同步的方式。）推荐使用异步的方式，
 * 以避免阻塞算子处理数据。
 * </p>
 *
 * <p>
