...
public class Task
     ...
     * 用于设置该并行实例属于哪个JobVertex
     * */
    private final JobVertexID vertexId;

    /** The execution attempt of the parallel subtask.
     * 表示该任务对应的Execution对象的唯一标识 */
     ...
     * 如果成功创建了Task对象并将其添加到了TaskExecutor的taskSlotTable中，就会掉用这个方法 */
     ...
     * the Environment.
     *  它表示在任务管理器端真正执行业务逻辑的任务，它的实现类包括StreamTask和BatchTask，分别代表流处理任务和批处理任务，
     *  它的初始化调用的Task类的loadAndInstantiateInvokable方法，它的执行调用的是AbstractInvokable的invoke方法，
     *  所有业务逻辑都在这个方法中实现。
