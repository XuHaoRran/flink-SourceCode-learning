 ...
 * 该对象保存了算子的计算逻辑、物理出边、链接在一起的算子的StreamConfig等信息，这些信息是在生成JobGraph的过程中设置的，
 * 有些信息被序列化成byte数组，还有一些原始类型的信息会直接以原始类型保存在其中。StreamConfig中又一个Configuration类型的
 * config字段，StreamConfig的所有信息都保存在其中
 */
@Internal
