 ...
 * StreamGraph的核心方法主要作用就是构造streamNode对象并将其放入streamnodes字段中进行维护，同时添加虚拟节点放入virtualPartitionNodes字段中进行维护，并且StreamEdge对象为节点添加出入边。
 */
@Internal
public class StreamGraph implements Pipeline {
    ...
    // 用于设置是否开启链接。如果开启，则可能实现把多个算子链接在一起，这样就可以把它们部署到一个任务中执行
    ...
    // 用于设置是否把所有节点放入到同一个SlotSharingGroup，默认是true
    ...
    public <IN, OUT> void addOperator(
            Integer vertexID, // vertexID就是transformationId
            @Nullable String slotSharingGroup,
            @Nullable String coLocationGroup,
            StreamOperatorFactory<OUT> operatorFactory,
            TypeInformation<IN> inTypeInfo,
    ...
    protected StreamNode addNode(
            ...
            String operatorName) {
        // 在addNode()方法中，根据传入的参数创建了StreamNode对象并将其放入streamNodes字段中维护起来。这里要注意vertexClass参数。在addOperator方法中，
        // 会根据任务是source任务还是后面只有一个输入的任务来分别传入SourceStreamTask.class或OneInputStreamTask.class。这个参数最终影响到TaskManager的任务在哪个类下被执行。
