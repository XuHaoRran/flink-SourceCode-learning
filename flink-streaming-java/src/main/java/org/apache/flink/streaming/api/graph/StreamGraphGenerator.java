 ...
 *这将从水槽开始遍历变换树。在每次转换时，我们递归地转换输入，然后在StreamGraph中创建一个节点，并
 * 将输入节点的边添加到新创建的节点。转换方法返回StreamGraph中表示输入转换的节点的ID。
 * 可以返回几个ID来处理反馈转换和联合。
 ...
 * 不会在StreamGraph中创建实际节点。对于这些，我们在StreamGraph中创建一个虚拟节点，该节点保存特定属性，即分区、选择器等。当从虚拟节点到下游节点创建边时，
 * StreamGraph解析原始节点的id，并在图中创建具有所需属性的边。
 * 例如，如果您有此图：Map-1 -> HashPartition-2 -> Map-3
 *
 * 其中数字表示变换ID。我们首先一路向下递归。
 * Map-1被转换，即我们创建ID为1的StreamNode。然后，我们转换HashPartition，
 * 为此，我们创建ID为4的虚拟节点，该节点保存HashPartition属性。此转换返回ID 4。
 * 然后我们变换Map-3。我们添加边4->3。StreamGraph解析了ID为1的实际节点，
 * 并使用属性HashPartition创建和边缘1->3。不会在流图中创建实际节点。
 *
 */
@Internal
public class StreamGraphGenerator {
    ...
    public StreamGraph generate() {
        ...
        //    完成所有transformation对象的转换，填充streamgraph
    ...
    private Collection<Integer> transform(Transformation<?> transform) {
        // 如果alreadytransformed字段中已经包含该transoformation对象，则直接取出并返回
    ...
    private Collection<Integer> translate(
        ...
        // 通过调用这个方法，可以确定该算子的SlotSharingGroup。SlotSharingGroup在判断算子是否可以进行链接时被用到
        ...
        return shouldExecuteInBatchMode
                ...
                : translator.translateForStreaming(transform, context);  // 转换操作，与1.10版本的
        // transformOneInputTransform操作差不多
    }
     ...
     * 其中有一些转换需要用到determineSlotSharingGroup方法
     *
    ...
    private String determineSlotSharingGroup(String specifiedGroup, Collection<Integer> inputIds) {
        ...
        } else {  // 如果没有指定，则按照如下规则设定值
            String inputGroup = null;
            for (int id : inputIds) { // 遍历输入节点的id
                String inputGroupCandidate = streamGraph.getSlotSharingGroup(id); // 获取输入节点的SlotSharingGroup值如果之前没有赋过值，
                // 则将输入节点的值赋给当前节点，如果已经赋过值，且该值跟输入节点的值不同，则返回默认值
                if (inputGroup == null) {
