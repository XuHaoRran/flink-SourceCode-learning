...
public class StreamSourceContexts {
     ...
     * forward watermarks. onTimestampContext为所有的元素赋予-1作为时间戳，也就意味
     * 着永远不会向下游发送Watermark。使用Processing Time时使用此Context，使用Processing Time的
     * 时候向下游发送Watermark没有意义，在实际处理中，各个计算节点会
     * 根据本地时间定义触发器，触发执行Window类计算，而不是根据Watermark来触发。
     */
     ...
     * watermark emission.使 用 摄 取 时 间 （ Ingestion Time ） 的 时 候 ，
     ...
     * assigned here.
     * <p>使用事件时间（Event Time）的时候，ManualWatermarkContext
     * 不会产生Watermark，而是向下游发送透传上游的Watermark。</p>
     * **/
     ...
     *
     * <p>WatermarkContext定义了与Watermark相关的行为：
     * 1）负责管理当前的StreamStatus，确保StreamStatus向下游传递。
     * 2）负责空闲检测的逻辑，当超过设定的事件间隔而没有收到数据或者Watermark时，认为Task处于空闲状态。</p>
     */
