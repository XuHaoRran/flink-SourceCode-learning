...
public class WindowOperator<K, IN, ACC, OUT, W extends Window>
     ...
     * 窗口的状态，窗口内的元素都在其中维护。
     * 窗口中的元素并没有保存在Window对象中，而是维护在WindowState中，windowStateDescriptor则是创建了windowState
     * 所需要用到的描述符。Window对象在维护窗口内元素的过程中作为windowState字段的namespace。
     * 因为TimeWindow等实现类重写了equals方法和hashcode方法，所以即便每个元素再分配窗口时会创建不同的window对象，它们也仍然
     * 属于同一个namespace
     * */
    ...
    /**
     * 当有元素到来时，会调用WindowOperator的processElement方法
     * @param element
     * @throws Exception
     */
    @Override
