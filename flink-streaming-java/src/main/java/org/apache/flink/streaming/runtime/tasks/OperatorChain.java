 ...
 *  <p>根据算子与算子之间的边是不是物理出边分别构造两种不同的输出对象。如果是物理出边，则会构造RecordWriterOutput类的对象，
 *  否则会构造其他类型的Output实现类的对象</>
...
public abstract class OperatorChain<OUT, OP extends StreamOperator<OUT>>
    ...
    // 其中有一个RecordWriter类型的字段，用于将数据写入ResultPartition，表示数据的输出
    ...
    public OperatorChain(
        ...
        // 从配置信息中反序列化出StreamOperatorFactory对象，即创建StreamOperator的工厂类
        ...
        // 获取链接在一起的算子的最后一个算子的出边，并用该出边信息构造RecordWriterOutput对象
    ...
    private <T> WatermarkGaugeExposingOutput<StreamRecord<T>> createOutputCollector(
            StreamTask<?, ?> containingTask, // 表示这个算子所在的StreamTask对象
            StreamConfig operatorConfig, // 要构造的Output对象所对应的算子的StreamConfig信息
            Map<Integer, StreamConfig> chainedConfigs, // 链接在一起的每个算子的StreamConfig
            ClassLoader userCodeClassloader, // 用户代码的类加载器
            Map<IntermediateDataSetID, RecordWriterOutput<?>> recordWriterOutputs,
            List<StreamOperatorWrapper<?, ?>> allOperatorWrappers, // 所有算子
            MailboxExecutorFactory mailboxExecutorFactory) { // 用于创建MailboxExecutor对象的工厂类对象
        List<WatermarkGaugeExposingOutput<StreamRecord<T>>> allOutputs = new ArrayList<>(4);

        // create collectors for the network outputs
        // 为物理出边狗改造RecordWriterOutput对象
    ...
    private <IN, OUT> WatermarkGaugeExposingOutput<StreamRecord<IN>> createOperatorChain(
        ...
        // operators
        // 调用createOutputCollector方法构造下一个Output对象
        WatermarkGaugeExposingOutput<StreamRecord<OUT>> chainedOperatorOutput =
                createOutputCollector(
                        ...
                        allOperatorWrappers,
                        mailboxExecutorFactory);
        // 将上面构造的output对象作为参数传入这个方法来构造StreamOperator对象
    ...
    private <IN, OUT> WatermarkGaugeExposingOutput<StreamRecord<IN>> wrapOperatorIntoOutput(
            ...
            ClassLoader userCodeClassloader,
            OutputTag<IN> outputTag) {
        // 由于不是物理出边对应的Output对象，因此需要将上面构造的StreamOperator对象封装到当前要返回的Output对象中
