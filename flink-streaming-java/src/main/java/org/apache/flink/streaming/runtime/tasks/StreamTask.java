...
public abstract class StreamTask<OUT, OP extends StreamOperator<OUT>>
    ...
    /** The chain of operators executed by this task. 链接在一起的算子，在构建OperatorChain的过程中就构建了算子之间的驶入输出关系，
     * 比如一个StreamOperator对象中可能持有一个表示输出的对象，而该表示输出的对象中又持有下一个StreamOperator对象，以此将数据传递下去一次进行处理 */
    protected OperatorChain<OUT, OP> operatorChain;

    /** The configuration of this streaming task. 该流处理任务的配置 */
    protected final StreamConfig configuration;

    /** Our state backend. We use this to create a keyed state backend. 状态后端 */
    ...
    /** 该类封装了逻辑上基于mailbox的执行模型，当StreamTask开始真正执行业务逻辑时，会调用runMailboxLoop **/
        ...
        if (isRunning) {
            actionExecutor.runThrowing(
                    () -> {
                        ...
                        System.out.println("------------StreamTask.performCheckpoint->>subtaskCheckpointCoordinator.checkpointState----------------------");
