...
public class MailboxProcessor implements Closeable {
     ...
     * checkpoints, ...
     * 表示邮箱的抽线，它会接受不同的邮件，在不同的方法中会涉及从其中取出邮件进行处理
     */
     ...
     * processing.
     * 可以将其理解为整个MailboxProcessor的默认操作，具体而已经按就是处理输入数据
     */
     ...
     * 表示处理过程中的等待状态，提供resume方法用于让处理继续进行
     */
    ...
    public MailboxExecutor getMainMailboxExecutor() {
        // 表示邮箱的执行器，持有mailbox依赖，提供方法用于向邮箱添加邮件
