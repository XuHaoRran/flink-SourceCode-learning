...
public class PartitionTransformationTranslator<OUT>
    ...
    private Collection<Integer> translateInternal(
        ...
        for (Integer inputId : context.getStreamNodeIds(input)) { // 添加虚拟节点，因为重分区
            // 操作不是真正的任务，所以这里用虚拟节点VirtualPartitionNode来表示。这个节点包括重分区的一些信息、
            // 虚拟节点的id以及上游节点的id，这样，之后就可以通过虚拟节点的id来找到它的上一个真实节点的id
