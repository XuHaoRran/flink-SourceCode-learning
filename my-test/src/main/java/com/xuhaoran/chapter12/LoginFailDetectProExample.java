...
public class LoginFailDetectProExample {
    public static void main(String[] args) throws Exception {
        ...
        // 2.定义模式，连续三次登录失败
        Pattern<LoginEvent, LoginEvent> pattern = Pattern.<LoginEvent>begin("fail")  // 第一次登录失败的事件
                ...
                .times(3).consecutive(); // 严格紧邻的三次登录失败
        ...
        SingleOutputStreamOperator<String> warnStream = patternStream.process(new PatternProcessFunction<LoginEvent, String>() {
            @Override
            public void processMatch(Map<String, List<LoginEvent>> map, Context context, Collector<String> out) throws Exception {
                // 提取三次登录失败事件
                ...
                out.collect(firstFailEvent.userId + "连续3次登录失败！登录时间：" +
